    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                state['step'] += 1
                from _torch_ipex import adam_step
                adam_step(exp_avg, exp_avg_sq, p_data_fp32, grad, beta1, beta2, group['eps'], state['step'], group['lr'], group['weight_decay'] != 0, group['weight_decay'], False)
                ...
                if p.data.dtype in {torch.float16, torch.bfloat16}:
                    print("before copy", flush=True)
                    p.data.copy_(p_data_fp32)
                    print("finish copy", flush=True)
