...
do
    ...
    fairseq-train $DATASET \
       ...
       --label-smoothing 0.1 --max-tokens $MAX_TOKEN \
       $ARGS --validate-training-performance  --performance-begin-its 30 --performance-its-count 50 \
       2>&1 |tee training_cpu_max_token_${MAX_TOKEN}_inst_$(($i*$NUM_NUMA+$j*$INST_PER_NODE)).log &  
done
done
